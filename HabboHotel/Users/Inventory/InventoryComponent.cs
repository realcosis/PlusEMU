...
public class InventoryComponent
{
    ...
    public void UpdateItems(bool fromDatabase)
    {
        if (fromDatabase)
            Init();
        if (_client != null) _client.SendPacket(new FurniListUpdateComposer());
    }
    ...
    public Item AddNewItem(int id, int baseItem, string extraData, int group, bool toInsert, bool fromRoom, int limitedNumber, int limitedStack)
    {
        if (toInsert)
        {
            ...
            else
            {
                ...
                if (id > 0)
                {
                    dbClient.RunQuery("INSERT INTO `items` (`id`,`base_item`, `user_id`, `limited_number`, `limited_stack`) VALUES ('" + id + "', '" + baseItem + "', '" + _userId + "', '" +
                                      limitedNumber + "', '" + limitedStack + "')");
                }
                else
                {
                    dbClient.SetQuery("INSERT INTO `items` (`base_item`, `user_id`, `limited_number`, `limited_stack`) VALUES ('" + baseItem + "', '" + _userId + "', '" + limitedNumber + "', '" +
                                      limitedStack + "')");
    ...
    public void RemoveItem(int id)
    {
        ...
        if (_floorItems.ContainsKey(id)) _floorItems.TryRemove(id, out var _);
        if (_wallItems.ContainsKey(id)) _wallItems.TryRemove(id, out var _);
    ...
    public bool TryAddPet(Pet pet) => _petsItems.TryAdd(pet.PetId, pet);
